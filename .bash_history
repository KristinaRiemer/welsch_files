...
cat
cat .bash_profile 
RStudio.version()  echo 7F503920 && /usr/bin/env && echo 3AC2A76F
uit()
pwd
cd pecan
ls
...
pwd
git status
git checkout -b fix_posterior_null
git status
git branch
rm -r biocro_darpa_results/
pwd
...
make install
git status
git checkout -- base/all/DESCRIPTION
git status
git diff
make install
pwd
...
git remote -v
git branch
git checkout -b fix_empty_parameters
git checkout develop
git branch
git branch -d fix_empty_parameters
git branch
git branch -d fix_empty_posteriors
git checkout -b fix_empty_posteriors
git branch
echo $R_LIB
echo $R_LIBS
cd ..
